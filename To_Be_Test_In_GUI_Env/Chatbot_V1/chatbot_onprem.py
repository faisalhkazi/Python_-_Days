...
def build_prompt(question, document_chunks, max_chunks=5):
...
"""
        ...
        # Limit document chunks to the first N (e.g., 5)
...
def get_llm_response(prompt, model="llama3", api_url="http://localhost:11434/v1/completions"):
    print(f"[DEBUG] Sending request to: {api_url} with model: {model}")
    ...
    try:
        ...
        data = response.json()
        print(f"[DEBUG] Raw LLM response: {json.dumps(data, indent=2)}")
...
if __name__ == "__main__":
    ...
    # Call on-prem LLM
    result = get_llm_response(prompt, model="llama3")
